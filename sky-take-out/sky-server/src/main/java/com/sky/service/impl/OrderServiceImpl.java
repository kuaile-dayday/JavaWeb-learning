...
public class OrderServiceImpl implements OrderService {
    ...
    * */
    ...
    public void userCancelById(Long id) {
        Orders ordersDB = orderMapper.getById(id);//查询是否有当前订单
        if(ordersDB == null){
    ...
    * */
    ...
    private String getOrderDishesStr(Orders orders) {
        ...
        // 这里.stream 是将列表转换成流 便于进行函数式操作
        // .map 对列表中每个元素（用x表示） 执行以下操作：获取x的名字 数量 然后拼接成 name*number的格式，由orderDish保存该字符串
        // .collect 即将转换后的字符串又收集到新的列表中 orderDishList

        // 将该订单对应的所有菜品信息拼接在一起
        return String.join("", orderDishList);
        // .join() 方法将列表中的元素用指定的分隔符连接起来，返回一个字符串。这里的分隔符为空"" 即是没有分隔符，如果是空格或者其他的，就是其他的效果
    }
